import { signIn } from 'next-auth/react'
import { useRouter } from 'next/router'
import * as React from 'react'
import styles from './signin.module.css'

export default function SignInPage() {
  const router = useRouter()
  const { callbackUrl } = router.query

  const handleSignIn = (provider: string) => {
    signIn(provider, { callbackUrl: (callbackUrl as string) || '/onboarding' })
  }

  return (
    <div className={styles.root}>
      <div className={styles.card}>
        <h1 className={styles.title}>로그인</h1>
        <p className={styles.desc}>소셜 계정으로 간편하게 시작하세요</p>

        <div className={styles.buttons}>
          <button
            className={`${styles.btn} ${styles.google}`}
            onClick={() => handleSignIn('google')}
          >
            <svg width='18' height='18' viewBox='0 0 18 18' fill='none'>
              <path d='M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615Z' fill='#4285F4'/>
              <path d='M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18Z' fill='#34A853'/>
              <path d='M3.964 10.706A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.706V4.962H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.038l3.007-2.332Z' fill='#FBBC05'/>
              <path d='M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.962L3.964 7.294C4.672 5.163 6.656 3.58 9 3.58Z' fill='#EA4335'/>
            </svg>
            Google로 계속하기
          </button>

          <button
            className={`${styles.btn} ${styles.kakao}`}
            onClick={() => handleSignIn('kakao')}
          >
            <svg width='18' height='18' viewBox='0 0 18 18' fill='none'>
              <path d='M9 0C4.029 0 0 3.121 0 6.973c0 2.477 1.647 4.643 4.132 5.886L3.1 16.488a.27.27 0 0 0 .406.29L8.2 13.9c.264.018.53.027.8.027 4.971 0 9-3.121 9-6.973S13.971 0 9 0Z' fill='#3C1E1E'/>
            </svg>
            카카오로 계속하기
          </button>

          <button
            className={`${styles.btn} ${styles.naver}`}
            onClick={() => handleSignIn('naver')}
          >
            <span className={styles.naverN}>N</span>
            네이버로 계속하기
          </button>
        </div>

        <button className={styles.back} onClick={() => router.back()}>
          돌아가기
        </button>
      </div>
    </div>
  )
}
